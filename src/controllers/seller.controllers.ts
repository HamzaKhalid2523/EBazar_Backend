...
class SellerController {
  getAllRecords = factoryController.getAll(SellerModel, null, "email");
  getOneRecord = factoryController.getOne(SellerModel);
  createNewRecord = factoryController.createOne(SellerModel);
  updateRecord = factoryController.updateOne(SellerModel);
  deleteRecord = factoryController.deleteOne(SellerModel);
  
  signupSeller = catchAsync(async (req: any, res: Response, next: NextFunction) => {
    const body = req.body;

    let doc = await SellerModel.find({"$or": [{'email': body.email}, {'phone': body.phone}]});
    doc = doc[0];
    ...
    let result = new SellerModel(body);
    await result.save();

    result.password = undefined;

    const uuid = uuidv4();
    const token = `seller-${result.email}-${uuid}`;
    const user = {
      token,
      data: result,
    };
  ...
  loginSeller = catchAsync(async (req: any, res: Response, next: NextFunction) => {
    ...
    let result = await SellerModel.find({"$or": [{'email': user}, {'phone': user}]}).select("+password");
    result = result[0];
  ...
  changePassword = catchAsync(async (req: any, res: Response, next: NextFunction) => {
    ...
    const user = await SellerModel.findOne({ _id: _id });
    if(!user) {
      return next(new AppError('This user doesnot exists.', 401));
    }
